...
def calculate_average_for_files(root_dir, prefix):
    ...
    # 루트 디렉토리 내 모든 폴더들에 대해 반복
    for folder_name in os.listdir(root_dir):
        folder_path = os.path.join(root_dir, folder_name)

        if os.path.isdir(folder_path):
            # 해당 폴더에 프리픽스+average_cpu_usage.txt와 프리픽스+average_calculation_time.txt가 있는지 확인
            cpu_usage_file = os.path.join(folder_path, f'{prefix}average_cpu_usage.txt')
            calc_time_file = os.path.join(folder_path, f'{prefix}average_calculation_time.txt')
